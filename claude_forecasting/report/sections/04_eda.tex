...
Statistical tests can disagree on short or noisy series, so a rolling-window view is a useful visual check. After first differencing, the rolling means of the four representative series hug zero and their rolling standard deviations flatten across the cutoff (\cref{fig:rolling_diff}) --- visually confirming the statistical jump from the previous subsection. The corresponding raw plots, omitted here for brevity, show the opposite: drifting means and bulging variances on the most-volatile series.
