...
\begin{frame}[t]{Timeline, Split, And Metric}
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{softcard}
      ...
      \softbullet{Later notebooks reuse one unified 80/20 per-series validation rule.}
    \end{softcard}
    ...
    \begin{whitecard}
      ...
      \end{center}
      \footnotesize
      A higher score is better. The weights \inlinecode{w\_i} mean that some rows contribute far more to the final score than others.
      \vspace{0.22em}
      {\scriptsize\textbf{Top 1\%}: 64.2\% \hfill \textbf{Top 5\%}: 92.6\% \hfill \textbf{Top 10\%}: 98.3\%}
    \end{whitecard}
  \end{columns}
\end{frame}
